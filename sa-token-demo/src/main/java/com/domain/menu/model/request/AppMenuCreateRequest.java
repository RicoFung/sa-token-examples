...
@Schema(description = "AppMenuCreateRequest 新增入参")
public class AppMenuCreateRequest implements Serializable
{
	private static final long serialVersionUID = 1L;

    // 路由路径       db_column: PATH 
	@Schema(title = "路由路径", example = "")
	@NotNull(message = "路由路径必传！")
	...
	@NotNull(message = "路由名称（必须唯一并且和当前路由component字段对应的页面里用defineOptions包起来的name保持一致）必传！")
	@NotBlank(message = "路由名称（必须唯一并且和当前路由component字段对应的页面里用defineOptions包起来的name保持一致）不能为空！")
	private String name;
    // 路由重定向       db_column: REDIRECT 
	@Schema(title = "路由重定向", example = "")
	@NotNull(message = "路由重定向必传！")
	@NotBlank(message = "路由重定向不能为空！")
	private String redirect;
    // 按需加载需要展示的页面       db_column: COMPONENT 
	@Schema(title = "按需加载需要展示的页面", example = "")
	@NotNull(message = "按需加载需要展示的页面必传！")
	@NotBlank(message = "按需加载需要展示的页面不能为空！")
	...
	@NotNull(message = "是否缓存该路由页面（开启后，会保存该页面的整体状态，刷新后会清空状态）必传！")
	@NotBlank(message = "是否缓存该路由页面（开启后，会保存该页面的整体状态，刷新后会清空状态）不能为空！")
	private Boolean metakeepalive;
    // 需要内嵌的iframe链接地址       db_column: METAFRAMESRC 
	@Schema(title = "需要内嵌的iframe链接地址", example = "")
	@NotNull(message = "需要内嵌的iframe链接地址必传！")
	@NotBlank(message = "需要内嵌的iframe链接地址不能为空！")
	private String metaframesrc;
    // 内嵌的iframe页面是否开启首次加载动画       db_column: METAFRAMELOADING 
	@Schema(title = "内嵌的iframe页面是否开启首次加载动画", example = "")
	@NotNull(message = "内嵌的iframe页面是否开启首次加载动画必传！")
	@NotBlank(message = "内嵌的iframe页面是否开启首次加载动画不能为空！")
	...
	@NotNull(message = "当前菜单名称或自定义信息禁止添加到标签页必传！")
	@NotBlank(message = "当前菜单名称或自定义信息禁止添加到标签页不能为空！")
	...
	@NotNull(message = "显示在标签页的最大数量，需满足后面的条件：不显示在菜单中的路由并且是通过query或params传参模式打开的页面。在完整版全局搜dynamicLevel即可查看代码演示必传！")
	@NotBlank(message = "显示在标签页的最大数量，需满足后面的条件：不显示在菜单中的路由并且是通过query或params传参模式打开的页面。在完整版全局搜dynamicLevel即可查看代码演示不能为空！")
	...
	@NotNull(message = "将某个菜单激活（主要用于通过query或params传参的路由，当它们通过配置showLink: false后不在菜单中显示，就不会有任何菜单高亮，而通过设置activePath指定激活菜单即可获得高亮，activePath为指定激活菜单的path）必传！")
	@NotBlank(message = "将某个菜单激活（主要用于通过query或params传参的路由，当它们通过配置showLink: false后不在菜单中显示，就不会有任何菜单高亮，而通过设置activePath指定激活菜单即可获得高亮，activePath为指定激活菜单的path）不能为空！")
	private String activepath;
    // 父级主键       db_column: PID 
	@Schema(title = "父级主键", example = "")
	@NotNull(message = "父级主键必传！")
